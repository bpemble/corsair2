...
### 3.3 Strike scope — ASYMMETRIC OTM + ATM (v1.3)

**Target**: OTM + ATM nickel strikes only, asymmetric on each side.

- **Calls**: ATM to ATM+5 nickel strikes (6 strikes)
- **Puts**: ATM-5 nickel strikes to ATM (6 strikes)
- **ATM strike quoted on both calls AND puts** (shared symmetry)
- Total unique strikes: **11** (ATM + 5 on each side of the grid)
- Total instruments quoted: **12** (11 strikes × {C or P},
  with ATM in both buckets)
- Max resting orders at any moment: **24** (each instrument has
  bid + ask per penny-jump policy)
- Recompute ATM every quote cycle (30s default); re-center strike
  window on material underlying moves

**Concrete example** at HG forward $6.00:
- ATM nickel: $6.00
- Calls quoted at: $6.00, $6.05, $6.10, $6.15, $6.20, $6.25
- Puts quoted at: $5.75, $5.80, $5.85, $5.90, $5.95, $6.00

**Not quoted**:
- DTE > 35
- Any ITM strike (**calls below ATM, puts above ATM** both excluded)
- Strikes further than 5 nickels from ATM (more than $0.25 OTM)
- Any non-nickel strike (penny grid points like $5.13, $5.27 ignored)
...
### 3.3a Why asymmetric OTM-only (v1.3)

**Optimizing for capital/margin efficiency, not for simulator-
measured unconstrained P&L.** This is an explicit tradeoff: the
symmetric scope (including ITM on both sides) produces higher
simulator P&L but consumes materially more SPAN per contract.
Under live margin binding, margin efficiency dominates because
margin is the binding constraint, not edge.

Restricted-backtest measurements (Phase 4 in-sample, 4mo):

| Scope | n_fills | Sharpe | P&L 4mo | Max DD | Margin cost per contract |
|---|---|---|---|---|---|
| Symmetric ATM ± 5 nickels | 3,772 | 6.53 | $356K | $6K | Higher (ITM weighted) |
| **Asymmetric OTM + ATM (THIS SPEC)** | **3,144** | **3.39** | **$126K** | **$45K** | **Lower (OTM only)** |

Simulator shows asymmetric worse on unconstrained metrics, but:
- Simulator does NOT model SPAN margin binding
- v1.1 spec measured 121-136% mean margin utilization at $250K on
  the SYMMETRIC pipeline — constraint-saturated most of the time
- Under binding, what matters is P&L per $ margin, and
  OTM options have approximately the same P&L/margin ratio as
  ITM while using ~40-50% less SPAN per contract
- Asymmetric produces a balanced short-strangle book by
  construction → additional SPAN netting benefit (~30-35% relief)

**Live reality may differ.** The unmodeled effects could work
either direction. Stage 1.5 is the calibration stage for this
trade-off (see §11b).

### 3.3b Phase 4 backtest did NOT apply this filter

The projection numbers in §13 come from a re-run of the Phase 4
backtest WITH the asymmetric filter applied post-hoc to the
existing fill set. Numbers are measurements, not estimates:
- Asymmetric active fills: 3,144 (from unrestricted 30,292
  rank-1 fills, 10.4% retention)
- Asymmetric gross edge: $853K measured
- Asymmetric MC Sharpe: 3.39 measured
- Asymmetric median P&L (4mo): $126K measured

Before OOS: don't trust these numbers as forward projection.
After Stage 1.5: live data calibrates whether asymmetric's
margin efficiency dominates symmetric's higher simulator P&L.
...
## 6b. Margin-aware tiered quote management (added v1.3)

Even with asymmetric OTM scope, margin will still bind at Stage
2 capital levels. The strategy must degrade gracefully rather
than hit binding events as hard halts. Tiered quote management:

| Margin utilization | Quote behavior | Instruments active |
|---|---|---|
| < 70% of cap | **Full quoting** at all asymmetric strikes | 12 |
| 70-85% | **Drop deep OTM first** (strikes ≥ ±4 nickels from ATM) | 8 |
| 85-95% | **Drop near-ATM** (strikes within ±2 nickels from ATM) — quote only mid-OTM (±3) | 4 |
| 95-100% | **Improving-fill only** (reduce existing positions; no new opens) | — |
| > 100% | Tier-1 margin kill; flatten book | — |

Rationale:
- Deep OTM strikes have the lowest theta per contract → reducing
  them first preserves the highest-theta-per-SPAN fills when
  margin is near binding
- Near-ATM gets dropped at the 85% threshold because it's the
  highest-delta area in the OTM scope → highest SPAN per contract
- At 95%+ only improving-fill activity allowed (which the Tier 2
  improving-fill exception already handles)

This differs from Stage 1 smoke test where simpler behavior is
used. Stage 1.5 is the first stage where tiered management is
activated.

## 6c. Inventory balance monitor (added v1.3)

Phase 4 asymmetric rerun showed **32.7% short-C vs short-P
imbalance** (1272 short calls vs 646 short puts) — because the
in-sample window had upward underlying drift and calls filled
faster than puts. This is a real live risk: the asymmetric scope
doesn't automatically produce a balanced book.

**Live metric**: `imbalance = |short_C_contracts - short_P_contracts|
/ (short_C + short_P)`

**Thresholds**:
- imbalance < 15%: normal, no action
- imbalance 15-30%: **preferentially quote underweight side** —
  tighten bid/ask on C or P side (whichever is underweight) by 1
  tick more aggressively to attract rebalancing flow
- imbalance > 30%: **widen overweight side by 1 tick** AND narrow
  underweight side — active rebalancing
- imbalance > 50%: **halt new opens on overweight side** until
  imbalance recovers to < 30%

Implementation: monitored as a 5-minute rolling metric. Not a
kill switch — operational skew adjustment.
...
### 13.3 Forward 12-month projection — v1.3 asymmetric OTM baseline

**Explicit design tradeoff**: the asymmetric OTM+ATM strike scope
optimizes for MARGIN EFFICIENCY, not simulator-measured
unconstrained P&L. The simulator cannot reward SPAN netting or
low-per-contract-margin configurations. The base case numbers
below reflect simulator P&L with a REDUCED margin-binding
haircut because the asymmetric scope is inherently more margin-
efficient.

Asymmetric restricted baseline (measured):
- Sharpe 3.39 (daily, 20 MC paths)
- P&L median $126K / 4mo → $378K/yr annualized
- Max DD $45K
- n_fills 377 (MC) / 3144 (raw active)
- Gross edge $271/fill

At $250K margin / $500K capital, sustained ~35-40 fills/day
(nickel+asymmetric), margin-aware tiered quote management:

| Metric | Point | ±1σ | ±2σ |
|---|---|---|---|
| Annual P&L | **$190K** | ±$130K | ±$240K |
| IRR on $500K capital | **38%** | ±26% | ±48% |
| Annual Sharpe (blended regime) | **1.4** | ±0.9 | ±1.7 |
| Annual max drawdown | $50K-$100K | — | up to $200K in stress |

**95% CI:**
- Annual P&L: [−$50K, $430K]
- Annual Sharpe: [−0.3, +3.1]

Compression table (compounded, starting from ASYMMETRIC baseline
$378K/yr Sharpe 3.39):

| Factor | Compression | Notes |
|---|---|---|
| Regime normalization (63:41 → 50:50) | −30% | Vega leg of inventory MTM |
| In-sample overfit | −25% | Standard OOS haircut |
| **Margin / SPAN binding** | **−10%** | **Half of symmetric's −20% because asymmetric is more margin-efficient** |
| Per-strike AS (nickel-filtered) | −5% | Below 15% MM-lit threshold at median |
| Execution slippage | −5% | Simulator vs live |
| Cumulative (compounding) | **~50%** | Asymmetric baseline × 0.50 → base |

Base P&L: $378K × 0.50 ≈ **$190K**. Rounded with uncertainty
buffer.

### 13.3a Capital-sizing variants at Stage 2

Because asymmetric scope uses less margin per fill, Stage 2
capital may be smaller than v1.1's $250K target. Variants to
decide based on Stage 1.5 live margin data:

| Variant | Margin cap | Total capital | Expected IRR | When to choose |
|---|---|---|---|---|
| A | $250K | $500K | 38% ± 26% | If Stage 1.5 shows binding > 40% of time |
| **B** | **$200K** | **$400K** | **48% ± 33%** | **Default — same $ P&L on less capital** |
| C | $150K | $300K | 63% ± 43% | If Stage 1.5 shows binding < 20% at $150K |

Variant B (Stage 2 at $200K) is the default if Stage 1.5 evidence
doesn't push toward A or C. IRR advantage comes from same
absolute P&L on smaller capital base.

### 13.3b Comparison to v1.2 symmetric projection

| Metric | v1.2 (symmetric, included ITM) | v1.3 (asymmetric OTM only) |
|---|---|---|
| Simulator Sharpe | 6.53 | 3.39 |
| Simulator P&L (4mo) | $356K | $126K |
| Base-case P&L (annual) | $400K | $190K |
| IRR on $500K | 80% | 38% |
| **IRR at Variant B ($400K)** | n/a | **48%** |
| **IRR at Variant C ($300K)** | n/a | **63%** |

The tradeoff: v1.3 gives up simulator P&L for margin efficiency
that matters in live. If the live-vs-sim ratio for asymmetric is
meaningfully better than for symmetric (SPAN netting, lower
binding frequency), asymmetric wins in realized P&L despite
worse simulator numbers. Stage 1.5 calibrates this.

These are research projections. Live Stage 1-2 data tightens the
band; first σ-halving happens after ~5 weeks of live paper.

### 13.4 Scenario labels (v1.3 — asymmetric OTM baseline)

All numbers annual, at Stage 2 steady state, Variant A ($500K
capital). Variants B/C scale IRR proportionally.

- **Strong** (15%): margin rarely binds (< 20%), regime calm,
  live live-vs-sim ratio is > 1.2× for asymmetric (the
  unmodeled SPAN-netting benefit materializes). Sharpe 2-3,
  IRR 70-110%, annual ~$350-550K.
- **Base** (50%): margin binds ~30% of the time, live ≈ sim.
  Sharpe 1-2, IRR 20-60%, annual ~$100-300K.
- **Conservative** (25%): margin binds heavily, capture rate
  drops, per-strike AS mild. Sharpe 0.5-1, IRR 10-30%, annual
  ~$50-150K.
- **Adverse** (10%): regime shift or per-strike AS spikes or
  live divergence bad. Sharpe < 0.5, IRR 0-15%, annual
  $0-$75K. Break-even worst case.

**At Variant B ($400K capital), IRRs improve ~25% across all
scenarios. At Variant C ($300K), ~65% across all scenarios.**
Whether the smaller capital is appropriate depends on Stage 1.5
live margin data.

Probabilities unchanged from v1.2 — v1.3 revises MAGNITUDE and
adds explicit capital-variant sensitivity. Strong case is
narrower than v1.2 because the simulator doesn't credit the
SPAN-netting benefit; live is where that shows up, and Stage
1-1.5 is the evidence-gathering phase.

The operational priority remains: move probability mass from
Adverse toward Base via cheap live-data evidence during Stage
1-2.
...
**HG front-only, CME HXE, DTE ≤ 35, ASYMMETRIC OTM + ATM nickel
strikes (calls ATM to ATM+5 nickels; puts ATM-5 nickels to ATM;
ATM shared), penny-jump quoting at min_edge 0.001, no hedging at
Stage 1-1.5, margin-aware tiered quote management (degrade OTM
progressively as util rises), inventory balance monitor
(rebalance at 15%+ skew), ramp $100K → $150K → $200K-$250K margin
over 6-8 weeks (Stage 1 smoke → Stage 1.5 SPAN calibration →
Stage 2 capital-variant decision), TAM-enforced scaling ceiling
148 fills/day, asymmetric-baseline base case Sharpe 1.4 ± 0.9
and IRR 38% (Variant A) / 48% (Variant B) / 63% (Variant C)
depending on final capital sizing at Stage 2.**
...
| **1.3** | 2026-04-18 | **Margin-aware v1.3**: strike scope flipped from symmetric ATM±5 to **ASYMMETRIC OTM + ATM only** (calls ATM to ATM+5; puts ATM-5 to ATM; ATM shared). Explicit design choice: optimizing for margin efficiency over simulator-measured unconstrained P&L. Simulator numbers are WORSE (Sharpe 3.39, $126K/4mo, DD $45K vs v1.2 symmetric 6.53/$356K/$6K) but the config is margin-efficient by construction (lower SPAN per contract, balanced short-strangle structure, SPAN-netting benefit the simulator can't credit). Base case revised: Sharpe 1.4 ± 0.9, IRR 38% ± 26% (Variant A $500K), $190K ± $130K/yr. Added capital-sizing variants B ($400K → IRR 48%) and C ($300K → IRR 63%) as Stage 1.5-data-driven decisions. Added §6b margin-aware tiered quote management (degrade OTM progressively as util rises) and §6c inventory balance monitor (rebalance at 15%+ short-C-vs-short-P skew). Live-vs-sim ratio is the evidence that validates or invalidates the asymmetric bet. |
